...
This is not recommended unless you know what you are doing. There exists a Nix flake dev environment available.
Recommended if you are familiar with Nix. Just run `nix develop` to get required dependencies.
...
There exists a reasonably comprehensive test suite.

```
epmd -daemon # need to start epmd daemon for cluster tests
mix test
```

Benchmarks for order handling process can be run using:
