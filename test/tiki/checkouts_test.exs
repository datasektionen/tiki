defmodule Tiki.CheckoutsTest do
  ...
  describe "stripe_checkouts" do
    ...
    test "create_stripe_checkout/1 with valid data creates a stripe_checkout" do
      valid_attrs = %{currency: "some currency", payment_intent_id: "some payment_intent_id", payment_method_id: "some payment_method_id", status: "some status"}

      assert {:ok, %StripeCheckout{} = stripe_checkout} = Checkouts.create_stripe_checkout(valid_attrs)
    ...
    test "update_stripe_checkout/2 with valid data updates the stripe_checkout" do
      stripe_checkout = stripe_checkout_fixture()
      update_attrs = %{currency: "some updated currency", payment_intent_id: "some updated payment_intent_id", payment_method_id: "some updated payment_method_id", status: "some updated status"}

      assert {:ok, %StripeCheckout{} = stripe_checkout} = Checkouts.update_stripe_checkout(stripe_checkout, update_attrs)
    ...
    test "update_stripe_checkout/2 with invalid data returns error changeset" do
      stripe_checkout = stripe_checkout_fixture()
      assert {:error, %Ecto.Changeset{}} = Checkouts.update_stripe_checkout(stripe_checkout, @invalid_attrs)
    ...
    test "delete_stripe_checkout/1 deletes the stripe_checkout" do
      ...
      assert_raise Ecto.NoResultsError, fn -> Checkouts.get_stripe_checkout!(stripe_checkout.id) end
    end
